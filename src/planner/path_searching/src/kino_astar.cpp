...
{
// 这段代码是KinoAstar类的成员函数setParam，用于从ROS参数服务器获取配置参数，并设置相应的成员变量。
// 此外，该函数还负责订阅点云话题和发布路径节点话题。以下是对这个函数每一部分的详细注释：
void KinoAstar::setParam(ros::NodeHandle& nh)
{
  ...
  // 这些行代码获取机器人的半径robot_r_和高度robot_h_参数。
  nh.param("kino_se3/robot_r", robot_r_, 0.2);
  nh.param("kino_se3/robot_h", robot_h_, 0.1);
  // 这行代码订阅名为"local_cloud"的sensor_msgs::PointCloud2话题，队列大小为10，当新消息到来时，将调用localCloudCallback成员函数进行处理。
...
// 这段代码是KinoAstar类的成员函数localCloudCallback，它是一个回调函数，用于处理从sensor_msgs::PointCloud2消息中接收到的局部点云数据。
// 这个函数的主要作用是将接收到的点云数据转换为PCL点云格式，并更新KD树（K-Dimensional Tree）以用于快速空间搜索。
// 以下是对这个函数每一部分的详细注释：
...
// 这是一行注释，说明这个函数的目的是将障碍物点转换为PCL点云。
